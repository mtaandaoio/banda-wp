...
<table class="wc_status_table widefat" cellspacing="0" id="status">
	<thead>
		<tr>
			<th colspan="3" data-export-label="WordPress Environment"><h2><?php _e( 'WordPress Environment', 'banda' ); ?></h2></th>
		</tr>
	</thead>
	<tbody>
		<tr>
		...
		<tr>
			...
			<td class="help"><?php echo wc_help_tip( __( 'The version of WordPress installed on your site.', 'banda' ) ); ?></td>
			<td><?php bloginfo('version'); ?></td>
		</tr>
		<tr>
			<td data-export-label="WP Multisite"><?php _e( 'WP Multisite', 'banda' ); ?>:</td>
			<td class="help"><?php echo wc_help_tip( __( 'Whether or not you have WordPress Multisite enabled.', 'banda' ) ); ?></td>
		...
		<tr>
			...
			<td><?php
				...
				if ( $memory < 67108864 ) {
					echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%s - We recommend setting memory to at least 64MB. See: %s', 'banda' ), size_format( $memory ), '<a href="https://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP" target="_blank">' . __( 'Increasing memory allocated to PHP', 'banda' ) . '</a>' ) . '</mark>';
				} else {
		...
		<tr>
			...
			<td class="help"><?php echo wc_help_tip( __( 'Displays whether or not WordPress is in Debug Mode.', 'banda' ) ); ?></td>
			<td>
		...
		<tr>
			...
			<td class="help"><?php echo wc_help_tip( __( 'The current language used by WordPress. Default = English', 'banda' ) ); ?></td>
...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
			...
			<tr>
				...
				<td>
					...
					if ( version_compare( $mysql_version, '5.6', '<' ) ) {
						echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%s - We recommend a minimum MySQL version of 5.6. See: %s', 'banda' ), esc_html( $mysql_version ), '<a href="https://wordpress.org/about/requirements/" target="_blank">' . __( 'WordPress Requirements', 'banda' ) . '</a>' ) . '</mark>';
					} else {
		...
		<tr>
			...
			<td class="help"><?php echo wc_help_tip( __( 'The largest filesize that can be uploaded to your WordPress installation.', 'banda' ) ); ?></td>
		...
		<?php
			...
			foreach ( $check_pages as $page_name => $values ) {
				...
				if ( ! $page_id ) {
					...
					echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( 'Page visibility should be %spublic%s', 'banda' ), '<a href="https://codex.wordpress.org/Content_Visibility" target="_blank">', '</a>' ) . '</mark>';
					$error = true;
				} else {
				...
				foreach ( $scanned_files as $file ) {
					...
					if ( ! empty( $theme_file ) ) {
						...
						if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
							...
							$found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'banda' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
						} else {
